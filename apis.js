...
var criador = ['lzvxzz']; // Nome do criador
const key = ['lzvxzz', 'freeapi'] //apikey das apis
...
function getRandom(nans) {
 ...
 router.all('/loli', async (req, res) => {
   ...
   if(!cdapikey) return res.json(resposta.semkey)
  	if(!key.includes(cdapikey)) return res.json(keyinvalida)
    ...
    router.get('/download/mediafire', async(req, res, next) => {
     ...
     if(!cdapikey) return res.json(resposta.semkey)
      ...
      .then(e => {
        ...
        router.get('/iafree', async (req, res, next) => { 
          const { ia } = require('./ia')
          ...
          res.json({ 
            status: true,
            creator: 'Lzvxzz',
            tipo: "Inteligencia artifical sem key",
            resposta: "[ IA ]\n" + openai[0].resposta
          })
        })
